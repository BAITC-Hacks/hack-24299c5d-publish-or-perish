...
`index.html` — игровое оформление, `classic.html` — классическое оформление из main. Переключатель «Интерфейс» сохраняет прогресс в текущей вкладке. Анализ, сравнение и HTML-презентация основного режима доступны в обоих вариантах. Испытание доступно в `challenge-demo.html` и `classic-challenge.html`; его итоговый экран пока показывает расчётную модель, как в main.

Графика: [ART.md](ART.md). Реальные архивные границы пяти районов: [data/MAP-SOURCES.md](data/MAP-SOURCES.md).

### Локальный запуск с ИИ на Node.js

Нужен Node.js 22 или новее. Запустите `./start-local.ps1` (выбирает Deno, если доступен, иначе Node.js), либо `node serve-node.mjs`. Откройте http://127.0.0.1:8765/. Для ИИ скопируйте `.env.example` в `.env.local` и заполните `OPENAI_API_KEY` и `OPENAI_MODEL`. Ключ остаётся на сервере; файл настроек не входит в список публичных файлов. Python-сервер на 8080 показывает интерфейс и презентацию, но не обслуживает `/api/analyze`.
