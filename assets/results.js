...
const gameArtwork=document.body.dataset.interface==='game';
const uniqueArtwork=new Set(['M1','M2','M5','M6','M8','M9','M11','M12','M14']);
const picture=(direction,id)=>gameArtwork?`assets/art/${uniqueArtwork.has(id)?id:art[direction]}.png`:`ruiling-city/images/card-${art[direction]}.png`;
function overview(report){
 const r=report.current;
 return `<article class="result-slide hero-slide"><div class="hero-copy"><p class="slide-kicker">Ваш итог · 8 кварталов</p><h2>Пять решений.<br>Новая Астана.</h2><div class="hero-number">${fmt(r.score)}<small>балла</small></div><span class="delta-chip">${signed(r.scoreChangeFromBaseline)} к исходному</span><div class="budget-graphic"><div class="budget-ring" style="--spent:${r.cost}%"><b>${r.cost}<small>из 100</small></b></div><p>Распределённый бюджет<br><strong>${100-r.cost} ед. в резерве</strong></p></div><small>Прогноз на синтетических данных</small></div><img class="hero-image" src="${gameArtwork?'assets/art/social.png':'ruiling-city/images/esil.png'}" alt="Иллюстрация развития города"></article>`;
}
function decisionsPage(report){
 return `<article class="result-slide"><p class="slide-kicker">Ваш план действий</p><h2>Во что вы вложились</h2><div class="decision-gallery">${report.decisions.map((d,i)=>`<div class="decision-tile"><img src="${picture(MEASURES[d.id].direction,d.id)}" alt=""><div><small>0${i+1} · ${esc(d.district||'Весь город')}</small><h3>${esc(MEASURES[d.id].name)}</h3><span>${MEASURES[d.id].cost} ед. · ${MEASURES[d.id].lag} кв. до эффекта</span></div></div>`).join('')}</div></article>`;
}
function advicePage(report,r,index){
 if(!r)return `<article class="result-slide"><p class="slide-kicker">Возможности улучшения</p><h2>Одиночная замена не повысит балл</h2><p>Проверено ${report.evaluated} допустимых вариантов. Другой набор из нескольких мер может дать более высокий результат.</p><img class="advice-art" src="${picture('Сервисы')}" alt="Городские сервисы"></article>`;
 const c=r.components;
 return `<article class="result-slide"><p class="slide-kicker">Вариант замены ${index+1} · применить отдельно</p><h2>Ещё ${fmt(r.gain)} балла</h2><div class="swap-visual"><div><img src="${picture(MEASURES[r.removed.id].direction,r.removed.id)}" alt=""><small>Убрать</small><h3>${esc(r.removeLabel)}</h3></div><span aria-hidden="true">→</span><div><img src="${picture(MEASURES[r.added.id].direction,r.added.id)}" alt=""><small>Добавить</small><h3>${esc(r.addLabel)}</h3></div></div><div class="swap-numbers"><b>${fmt(r.score)}<small>новый балл</small></b><b>${r.cost}/100<small>бюджет</small></b></div><p class="formula-note">Вклад среднего: ${signed(c.average)} · минимума: ${signed(c.minimum)} · штрафа: ${signed(c.penalty)}</p><small>Одна замена в исходном наборе. Приросты вариантов не складываются.</small></article>`;
}
